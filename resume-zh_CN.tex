...
\begin{enumerate}[leftmargin=0.3in, label={[\arabic*]}]
  ...
  {ICML 2026 (CCF-A)}{}
  {\small\color{secondary} 将知识库问答建模为多轮 MDP，模型在与 KB 的闭环交互中基于环境反馈自适应调整推理策略；提出 RRS 解决 RL 冷启动，通过 GRPO 以 outcome-based reward 优化策略，三个基准均达 SOTA，推理效率较 MCTS 方法提升 26 倍。}
...
\section{实习经历}
  \resumeSubHeadingListStart
    \resumeSubheading
      ...
      \resumeItemListStart
        \resumeItem{RAG 核心方向研究：主导完成多项检索增强生成相关工作，包括 RAG 可信对齐 (ACL 2025)、多模态自适应 RAG (IJCNN 2026)、RAG 测试时自适应 (ICASSP 2026)、知识库问答强化学习 KBQA-R1 (ICML 2026) 等}
      \resumeItemListEnd
